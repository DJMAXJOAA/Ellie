...
namespace Managers.RangeIndicator.Range
{
    public class RectangleRange : BaseRange
    {
        ...
        public override List<Transform> CheckRange(string checkTag = "", int layerMask = -1)
        {
            ...
            var collidersInRectangle =
                Physics.OverlapBox(center, new Vector3(Width / 2, 1, Height / 2), direction, layerMask);

            // 이 콜라이더들 중에서 "적" 태그를 가진 것들만 선택합니다.
            var targets = new List<Transform>();
            foreach (var collider in collidersInRectangle)
            {
                if (checkTag == "" || collider.CompareTag(checkTag))
                {
                    // 이 콜라이더의 위치를 사각형의 로컬 좌표계로 변환합니다.
                    var localColliderPosition = objTransform.InverseTransformPoint(collider.transform.position);
