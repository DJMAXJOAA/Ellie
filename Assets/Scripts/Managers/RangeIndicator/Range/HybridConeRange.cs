...
namespace Managers.RangeIndicator.Range
{
    public class HybridConeRange : BaseRange
    {
        ...
        public override void CreateRange(RangePayload payload)
        {
            ...
            var segments = Mathf.CeilToInt(Angle);
        ...
        public override List<Transform> CheckRange(string checkTag = "", int layerMask = -1)
        {
            ...
            var lowerBase = 2 * Radius * Mathf.Sin(Mathf.Deg2Rad * Angle / 2);

            // 하이브리드 범위 내의 모든 콜라이더를 감지합니다.
            var collidersInHybrid = Physics.OverlapSphere(center, Radius, layerMask);

            var targets = new List<Transform>();
            foreach (var collider in collidersInHybrid)
            {
                if (checkTag == "" || collider.CompareTag(checkTag))
                {
                    ...
                    // 부채꼴 범위 내에 있는지 확인합니다.
                    if (localPoint.z >= 0 && localPoint.z <= Radius &&
                        Mathf.Abs(Mathf.Atan2(localPoint.x, localPoint.z) * Mathf.Rad2Deg) <= Angle / 2)
                    {
                    ...
                    {
                        var halfWidthAtThisPoint = Mathf.Lerp(UpperBase / 2, lowerBase / 2, localPoint.z / Radius);
